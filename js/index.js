...
btnDialogBaterPonto.addEventListener("click", async () => {
  ...
  const isDateAltered = selectedDate !== currentDate; // Verifica se a data foi alterada

  const hora =
    horaInput && horaInput.style.display !== "none" && horaInput.value
      ? horaInput.value
      : getCurrentHour();
  ...
  let ponto = {
    ...
    dataAlterada: isDateAltered, // Adiciona a propriedade dataAlterada corretamente
  };

  console.log("Registro criado:", ponto); // Confirma no console
...
function saveRegisterLocalStorage(register) {
  const typeRegister = document.getElementById("tipos-ponto");
  registerLocalStorage.push(register); // Adiciona o registro ao array
...
function register() {
  ...
  dialogHora.textContent = "Hora: " + getCurrentHour();
   // Armazenar a data original em um campo separado
   originalDate = document.getElementById("date-select").value;
